...
function GameUIShrineReport:RefreshFightListView()
	...
	for i,v in ipairs(self:GetShrineReport():FightDatas()) do
		...
		item:setItemSize(content:getCascadeBoundingBox().width,content:getCascadeBoundingBox().height+40)
...
function GameUIShrineReport:CreateIf_data_statistics()
	...
	local honour_icon = display.newSprite("honour.png"):addTo(logo):pos(286,15)
	UIKit:ttfLabel({
		text = _("联盟获得"),
		size = 18,
		color = 0xffedae
