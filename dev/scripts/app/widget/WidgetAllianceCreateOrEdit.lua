...
function WidgetAllianceCreateOrEdit:createCheckAllianeGroup()
	...
	if self:IsCreate() then
	    ...
	    UIKit:ttfLabel({text = _("草地"),size = 20,color = 0x615b44}):align(display.LEFT_CENTER,70,40):addTo(landSelect)
	    UIKit:ttfLabel({text = _("沙漠"),size = 20,color = 0x615b44}):align(display.LEFT_CENTER,270,40):addTo(landSelect)
	    UIKit:ttfLabel({text = _("雪地"),size = 20,color = 0x615b44}):align(display.LEFT_CENTER,470,40):addTo(landSelect)
		self.landTypeButton = cc.ui.UICheckBoxButtonGroup.new()
	        ...
	        :addButton(UICheckBoxButton.new(checkbox_image)
	            :align(display.LEFT_CENTER))
	        :setButtonsLayoutMargin(10, 140, 0,0)
