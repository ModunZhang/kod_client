...
local function HateDragon()
    ...
    if not check("HateDragon") then
        mark("HateDragon")
        ext.market_sdk.onPlayerEvent("孵化龙")
    end
end
local function DefenceDragon()
    ...
    if not check("DefenceDragon") then
        mark("DefenceDragon")
        ext.market_sdk.onPlayerEvent("驻防龙")
    end
end
...
local function BuildHouseAt(building_location_id, house_location_id, house_type)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("建造小屋:"..building_location_id.."_"..house_type)
    end
end
...
local function FinishUpgradingBuilding(type, level)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("升级建筑完成:"..type.."_"..level)
    end
end
local function UpgradeBuildingTo(type, level)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("升级建筑:"..type.."_"..level)
    end
end
...
local function RecruitSoldier(type_, count)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("招募士兵:"..type_)
    end
end
...
local function InstantRecruitSoldier(name, count)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("立即招募士兵:"..name)
    end
end
...
local function GetSoldier()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("获得士兵")
    end
end
...
local function FightWithNpc(floor)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("探索pve:"..floor)
    end
end
...
local function FinishTreatSoldier()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("治疗士兵完成")
    end
end
...
local function TreatSoldier(type_, count)
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("治疗士兵")
    end
end
...
local function FinishResearch()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("研发科技完成")
    end
end
...
local function Research()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("研发科技")
    end
end
...
local function CheckMaterials()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("查看材料")
    end
end
...
local function Skip()
    ...
    if not check(key) then
        mark(key)
        ext.market_sdk.onPlayerEvent("跳过引导")
    end
end
