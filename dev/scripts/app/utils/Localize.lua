...
local USER_AGREEMENT = {
    agreement = _("《Batcatstudio游戏用户使用许可协议》\n1、特别提示当您使用本软件及成都蝠猫科技有限公司(以下简称“蝠猫科技”)的服务时,即表示您同意遵守本协议及蝠猫科技为提供相应服务制订的相关规范,遵守中国法律法规的规定。蝠猫科技在为您提供相关客户服务的前提是您能表明您是账号的所有人,这可能需要您提供相关信息(包括但不限于注册信息、历史密码等),如果用户没有牢记自己填写的注册资料及相关历史信息或未及时更新 相关注册资料,您的相关问题(包括但不限于密码找回等)将得不到解决,对此蝠猫科技不承担任何责任。\n2\n2.1 用户同意访问个人基本信息包括:姓名、个人头像、性别、所属网络、用户 id、朋友列表以及任何我 与所有网名共享的内容;\n2.2 用户同意蝠猫科技可以通过电子邮件或其他方式向用户发送的商品促销或其他相关商业信息。\n2.3 用户在使用蝠猫科技服务过程中,必须遵循以下原则:\n2.3.1 遵守中国有关的法律和法规;\n2.3.2 不得为任何非法目的而使用网络服务系统;\n2.3.3 遵守所有与网络服务有关的网络协议、规定和程序;\n2.3.4 不得利用蝠猫科技服务系统进行任何可能对互联网的正常运转造成不利影响的行为;\n2.3.5 不得利用蝠猫科技服务系统传输任何骚扰性的、中伤他人的、辱骂性的、恐吓性的、庸俗淫秽的或其 他任何非法的信息资料;\n2.3.6 不得利用蝠猫科技服务系统进行任何不利于蝠猫科技的行为;\n2.3.7 就蝠猫科技及合作商业伙伴的服务、产品、业务咨询应采取相应机构提供的沟通渠道,不得在公众场 合发布有关蝠猫科技及相关服务的负面宣传。\n2.3.8 如发现任何非法使用用户账号或账号出现安全漏洞的情况,应立即通告蝠猫科技。\n2.4 用户根据本协议获得的用户账号和密码属于蝠猫科技所有，用户对用户账号及密码不具有所有关系。当用户使用该用户账号和密码登录蝠猫科技的网站或接受其他蝠猫科技提供的服务项目时,  应遵守该网站或服务项目的相关服务协议及使用守则, 用户登录上述网站或接受上述服务项目时即视为对相关服务协议及使用守则的接受。\n2.5 在不透露单个用户隐私资料的前提下,蝠猫科技有权对整个用户数据库进行技术分析并对已进行分析、 整理后的用户数据库进行商业上的利用。尽管蝠猫科技对用户的隐私权保护做了极大的努力,但是仍然不 能保证现有的安全技术措施使用户的技术信息等不受任何形式的损失。\n2.6  连续360天没有使用蝠猫科技所提供的服务（自用户最后一次使用服务之日起计算），则自第360天当天的24时起,蝠猫科技有权采取措施终止该用户继续使用服务，删除该用户帐号，包括但不限于注册信息、等级信息、角色信息、物品信息等一切与注册帐号相关的信息，且勿需对删除该帐号所带来的任何利益损失负责；\n\n3、服务变更、中断或终止\n3.1 发生下列情形之一时,蝠猫科技有权停止或中断线上游戏,并且不向任何人承担因此产生的任何责任; \n3.1.1 对于蝠猫科技的网络设备进行必要的保养及施工;\n3.1.2 发生突发性的网络设备故障时;\n3.1.3 由于蝠猫科技所用的网络通信设备由于任何原因停止,无法提供服务时;\n3.1.4 由于不可抗力因素致使蝠猫科技无法提供线上游戏服务;\n3.1.5 应政府机关的命令终止运营，游戏软件本身的技术问题导致无法继续运营。不管由于任何原因终止运营,蝠猫科技均会提前一个月通 知用户,用户应采取相应措施自行处理游戏帐号(包括注销或停止使用该帐号)、游戏内虚拟物品等相关事宜。用户不得以任何原因要求蝠猫科技不得全面终止运营,也不得因全面终止运营而要求蝠猫科技承担任 何形式的赔偿或补偿责任(包括但不限于不再能继续使用游戏帐号、游戏内虚拟物品等)。\n\n3.1.6 除 2.1 条规定之外,蝠猫科技有权停止或更改各项服务内容或终止任一用户账户,但需做官方通知。无论任何情形, 蝠猫科技就停止或更改或终止用户账户所可能产生的不便或损害, 蝠猫科技对用户本人或任何第三人均不负任何损害赔偿责任。\n\n4、知识产权 \n4.1.本软件产品中的各种虚拟物品数据, 包括但不限于资源、功勋、道具、装备等,其所有权归蝠猫科技公司所有, 用户只能在合乎法律规定和游戏规则的情况下使用。\n\n5、用户禁止行为或活动\n5.1 禁止用户发生以下侵害本网络游戏公平性的行为,包括但不限于:\n5.1.1 使用除AppStore及蝠猫官方支付渠道之外的非官方支付渠道来购买游戏内置虚拟物品的行为。以及传播，教唆他人使用此类购买渠道的行为。\n5.1.2 使用任何外挂程序或游戏修改程序(本协议所称“外挂程序”是指独立于游戏软件之外的,能够在游戏 运行的同时影响游戏操作的所有程序,包括但不限于模拟键盘鼠标操作、改变操作环境、修改数据等一切 类型。如国家有关法律、法规及政府主管部门的规章或规范性文件规定的外挂定义与本协议有冲突,则以 法律、法规、部门规章或规范性文件规定的为准),对本网络游戏软件进行还原工程、编译、译码或修改, 包括但不限于修改本软件所使用的任何专有通讯协议、对动态随机存取内存(RAM)中资料进行修改或锁 定;\n5.1.3 使用异常的方法登录游戏、使用网络加速器等外挂软件或机器人程式等恶意破坏服务设施、扰乱正常服务秩序的行为;\n5.1.4 制作、传播或使用外挂、封包、加速软件,及其它各种作弊程序,或组织、教唆他人使用此类软件程 序,或销售此类软件程序而为私人或组织谋取经济利益;\n5.1.5 使用任何方式或方法,试图攻击提供游戏服务的相关服务器、路由器、交换机以及其他设备,以达到 非法获得或修改未经授权的数据资料、影响正常游戏服务,以及其他危害性目的的任何行为;\n5.1.6 利用线上游戏系统可能存在的技术缺陷或漏洞而以各种形式为自己及他人牟利(包括但不限于复制游 戏中的虚拟物品等)；\n5.1.7 利用反向工程、编译或反向编译、反汇编等技术手段制作软件对游戏进行分析、修改、攻击,最终达 到作弊的目的。\n5.1.8 非法贩卖游戏内置物品，如金龙币，资源等行为，一但发现，将永久封停。\n5.1.9 每个用户的通行证只限用户自己使用，用户创建的角色不得转让、买卖，禁止利用论坛、信函、B2B或C2B等网站对俱乐部进行买卖，为此带来的任何损失，官方不提供任何服务和帮助。\n5.1.10 禁止使用冒充运营团队的游戏id，例如“游戏、GM、游戏客服”等容易误导和欺骗玩家的游戏id，一旦发现，将立刻永久封停。\n5.2 一旦蝠猫科技通过内部的监测程序发现或经其他用户举报而发现您有可能正在从事上述行为,则蝠猫科技有权采取相应的措施进行弥补,该措施包括但不限于限制您帐号和游戏中角色的登陆、限制您在游戏 中的活动、删除与复制有关的物品(包括复制出的虚拟物品和参与复制的虚拟物品)、封停或删除您的游戏 角色、帐号和要求您赔偿因您从事上述行为而给蝠猫科技造成的损失等。\n5.3 对反向工程(Reverse Engineering)、反向编译(Decompilation)、反汇编(Disassembly)的禁止。您不得对本“软 件产品”进行反向工程(Reverse Engineering)、反向编译(Decompile)或反汇编(Disassemble),您亦同意放弃行 使适用法律允许上述活动之权利。\n\n6、风险承担 用户同意使用蝠猫科技线上游戏软件是出于用户个人意愿,并愿自负任何风险,包括但不限 于其因执行或自行由官方网站下载游戏或资料图片而导致用户或其所使用相关 系统损害,或发生任何资料的流失等。\n\n7、免除责任\n7.1 蝠猫科技对任何直接、间接、偶然、特殊及继起的损害不负责任,这些损害可能来自:不正当使用网络 服务,非法使用网络服务或用户传送的信息有所变动等方面。用户明确同意其安装、复制、访问网站、充值、运行客户端软件或以其它方式使用“软件产品”及/或接受蝠猫科技提供的相关服务所存在的风险将完 全由其自己承担。\n7.2 蝠猫科技注册账号服务不保证以下事宜:\n(i) 本服务将符合您的要求;\n(ii) 本服务将不受干扰、及时提供、安全可靠或不会出错。\n7.3 用户明确同意其使用蝠猫科技服务所存在的风险将完全由其自己承担;因其使用蝠猫科技服务而产生 的一切后果也由其自己承担,蝠猫科技对用户不承担任何责任。\n8、服务条款的修改 由于用户及市场状况的不断变化,蝠猫科技保留随时修改本服务条款的权利,修改本服务条款时,蝠猫科技将于Batcatstudio官方网站首页公告修改的事实,而不另对用户进行个别通知。若用户不同意修改的内 容,可停止使用蝠猫科技的线上游戏。若用户继续使用蝠猫科技的线上游戏,即视为用户业已接受蝠猫科技所修订的内容。\n成都蝠猫科技有限公司对于本协议内容拥有最终解释权和修订权。\n成都蝠猫科技有限公司\n\n二〇一五年五月")
}
return {
