local NormalMapAnchorBottomLeft = class("NormalMapAnchorBottomLeft")
local floor = math.floor
local ceil = math.ceil
function NormalMapAnchorBottomLeft:ctor(map_info)
...
function NormalMapAnchorBottomLeft:ConvertToMapPosition(x, y)
    return x * self.tile_w + self.half_w, y * self.tile_h + self.half_h
end
function NormalMapAnchorBottomLeft:ConvertToLogicPosition(x, y)
    return floor((x - self.base_x) % 80), floor((y - self.base_y) % 80)
end
function NormalMapAnchorBottomLeft:ConvertToLocalPosition(x, y)
    return x * self.tile_w + self.half_w, y * self.tile_h + self.half_h
end



return NormalMapAnchorBottomLeft