...
void main(void) {
    ...
    float newT = curTime*0.4 + sn(vec2(curTime*1.))*0.1;
    p.x-=curTime*0.2;
    ...
    -.125*sn(4.03*p-curTime*0.3);
    // +.0625*sn(8.02*p-curTime*0.4)
    // +.03125*sn(16.01*p+curTime*0.5)+
    // .018*sn(24.02*p);
    ...
    +.25*sn(2.04*p+newT*1.1)
    -.125*sn(4.03*p-curTime*0.3);
    // +.0625*sn(8.02*p-curTime*0.5)
    // +.03125*sn(16.01*p+curTime*0.6)+
    // .019*sn(18.02*p);
    
    float f4 = f2*smoothstep(0.0,1.,uv.y);
    
    vec3 clouds = mix(vec3(-0.4,-0.4,-0.15),vec3(1.4,1.4,1.3),f4*f);
    float lightning = sn((f3)+vec2(pow(sn(vec2(curTime*4.5)),6.)));
    ...
    newUV.x-=curTime*0.3;
    newUV.y+=curTime*3.;
    float strength = sin(curTime*0.5+sn(newUV))*0.1+0.15;
    
    float rain = sn( vec2(newUV.x*20.1, newUV.y*40.1+newUV.x*400.1-20.*strength ));
    float rain2 = sn( vec2(newUV.x*45.+curTime*0.5, newUV.y*30.1+newUV.x*200.1 ));  
    rain = strength-rain;
