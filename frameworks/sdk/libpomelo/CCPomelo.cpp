...
CCPomelo::~CCPomelo(){
    Director::getInstance()->getScheduler()->unscheduleSelector(schedule_selector(CCPomelo::dispatchCallbacks), s_CCPomelo);
}
...
void CCPomelo::incTaskCount(){
    ...
    task_count++;
    pthread_mutex_unlock(&task_count_mutex);
    Director::getInstance()->getScheduler()->resumeTarget(s_CCPomelo);
}
