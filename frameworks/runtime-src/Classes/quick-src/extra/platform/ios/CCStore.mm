...
//dannyhe
void Store::updateTransactionStates()
{
    m_transactions.clear();
    [[StoreIOS sharedStore]updateTransactionStates];
}

bool Store::purchaseWithIdentifier(const char* productId,int count)
{
    if (!productId) return false;
    [[StoreIOS sharedStore] purchaseWithIdentifier:[NSString stringWithCString:productId encoding:NSUTF8StringEncoding] count:count];
    return true;
}
